    ...
    def run(self, data):
        ...
        if self.previous_price is not None:
            ...
            if price_change >= 0.05:
                target_allocation = 0
                log(f"Price increased by ${price_change}. Setting target allocation for {self.ticker} to {target_allocation} (SELL).")
            
            # If price is down by at least $0.05, buy (set allocation to 1).
            elif price_change <= -0.05:
                target_allocation = 1
                log(f"Price decreased by ${price_change}. Setting target allocation for {self.ticker} to {target_allocation} (BUY).")
